...
const WORD BlockCrc::crcLookupTable[] =
{
 ...
 *
 *  DESCRIPTION
 *      BlockCrc constructor
 *---------------------------------------------------------------------------*/
BlockCrc::BlockCrc(void)
{
    crcReset();
}
 ...
 *
 *  DESCRIPTION
 *      Adds a byte to the CRC from the lookup table
 *---------------------------------------------------------------------------*/
void BlockCrc::crcAddByte(BYTE byte)
{
    BYTE data;
    
    data = ((BYTE)reflect(byte, 8) ^ (mCrcRemainder >> 8));
    mCrcRemainder = crcLookupTable[data] ^ (mCrcRemainder << 8);
}
 ...
 *      Read the CRC
 *---------------------------------------------------------------------------*/
WORD BlockCrc::crcRead(void)
{
    return (mCrcRemainder ^ FINAL_XOR_VALUE);
}
