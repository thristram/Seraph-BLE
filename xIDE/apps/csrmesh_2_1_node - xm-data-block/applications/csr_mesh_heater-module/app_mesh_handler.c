...
#define TX_QUEUE_SIZE                  (8)  //data_stream 每包8个有效字节
#define DEFAULT_ADV_INTERVAL           (90 * MILLISECOND)  //每发一包间隔90ms
#define DEFAULT_ADV_TIME               (600 * MILLISECOND)  //600ms发一次数据 
...
#define DEVICE_REPEAT_COUNT            (DEFAULT_ADV_TIME)/(DEFAULT_ADV_INTERVAL\
                                        + ONE_SHOT_ADV_TIME)  //600ms发一次，每次里面又重复发6次
#define RELAY_REPEAT_COUNT             (DEVICE_REPEAT_COUNT / 2) //转发次数
...
static void setMeshConfigParams(CSR_SCHED_MESH_LE_PARAM_T *mesh_le_params)
{
    ...
    mesh_le_params->tx_param.device_repeat_count = DEVICE_REPEAT_COUNT; //发送数据重复次数
    mesh_le_params->tx_param.relay_repeat_count  = RELAY_REPEAT_COUNT;//转发次数
    mesh_le_params->tx_param.tx_queue_size       = TX_QUEUE_SIZE;   //每包8个有效字节
    mesh_le_params->tx_param.tx_queue_ptr        = tx_queue_buffer;//发送有效字节缓存
}
...
{
    generic_le_params->scan_param_type      = CSR_SCHED_SCAN_DUTY_PARAM; //扫描间隔
    generic_le_params->scan_param.scan_duty_param.scan_duty_cycle
                                            = HIGH_SCAN_DUTY_CYCLE; //高扫描间隔
    generic_le_params->scan_param.scan_duty_param.min_scan_slot   
                                            = DEFAULT_MIN_SCAN_SLOT;
    generic_le_params->advertising_interval = DEFAULT_ADV_INTERVAL; //默认广播间隔
    generic_le_params->advertising_time     = DEFAULT_ADV_TIME; ////默认广播时间
    generic_le_params->addr_type            = DEFAULT_ADDR_TYPE;//随机生产蓝牙地址
}
...
extern void AppMeshInit(void)
{
    ...
    setLeConfigParams(&le_params);  //广播及扫描初始化
    CSRSchedSetConfigParams(&le_params);//一个调度MESH和LE的函数

    /* Start ADV GATT Scheduler */
    CSRSchedStart(); //开启adv和le调度器
     ...
     */
    InitializeSupportedModelData(); //初始化各类model函数，目前除了data model有用到外，其他均没有使用

    InitialiseSensorData();//可屏蔽，目前没有使用到sensordata

    /* Read persistent storage */
    readPersistentStore();  //读各类寄存器数据

    /* Register with CSR Mesh */
#ifdef USE_AUTHORISATION_CODE
    result = CSRmeshInit(CSR_MESH_NON_CONFIG_DEVICE_WITH_AUTH_CODE);//有证书
#else
    result = CSRmeshInit(CSR_MESH_NON_CONFIG_DEVICE);//没有证书，初始化CSRMESH
#endif /* USE_AUTHORISATION_CODE */
    ...
    if(result == CSR_MESH_RESULT_SUCCESS)
    {
        ...
        initializeSupportedModels();//初始化所有的MODEL，这里除了data model外，其他可屏蔽

        /* Start CSRmesh */
        result = CSRmeshStart();//开户CSRMESH
