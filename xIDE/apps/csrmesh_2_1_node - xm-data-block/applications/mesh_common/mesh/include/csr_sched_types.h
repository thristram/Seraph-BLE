...
#if (CSR_MESH_ON_CHIP == 1) || defined(CSR_MESH_UT_SCHEDULER_ENABLE)
...
typedef enum
{
    ...
    CSR_SCHED_SCAN_DUTY_PARAM   = 0x01 /*!< \brief scan parameter contains scan duty cycle and min scan slot 扫描参数:包含扫描占空比和最小扫描时隙*/
...
/*! \brief CSR Mesh Sched TX queue buffer Type*/
typedef struct
{
    CsrUint16 msg[18]; 
} CSR_SCHED_MESH_TX_BUFFER_T;
...
typedef struct
{
    ...
    CSR_SCHED_MESH_TX_BUFFER_T *tx_queue_ptr; /*! < \brief transmit queue buffer pointer */
...
typedef enum
{
    ...
    CSR_SCHED_GATT_EARLY_WAKEUP_EVENT/*!< \brief GATT Early event wake-up*/
#endif
...
typedef enum
{
    ...
    CSR_SCHED_RESULT_TX_QUEUE_EMPTY          =0x00006, /* !< \brief Operation result TX Queue empty */
    CSR_SCHED_RESULT_UNACCEPTABLE_SCAN_DUTY_VALUE = 0x0007,  /* !< \brief Operation result unacceptable scan duty value*/
    CSR_SCHED_RESULT_RADIO_BUSY                = 0x0008,/* !< \brief Operation result radio is busy*/
    CSR_SCHED_RESULT_TX_QUEUE_NOT_INITIALIZED   = 0x0009, /* !< \brief Operation result TX queue not initialized*/
...
#endif /*(CSR_MESH_ON_CHIP == 1) || defined(CSR_MESH_UT_SCHEDULER_ENABLE)*/
